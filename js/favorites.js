  ...
  const generateLikedProduct = (data) => {
    return `
      <li class="favorites__item">
        <article class="favorites-product" data-id="${data.id}">
          <div class="favorites-product__row">
            <div class="favorites-product__stiker favorites-product__stiker--green">${data.stiker}</div>
          ...
          <div class="favorites-product__props">
            ...
            <span class="favorites-product__testimonials">${data.testimonials}</span>
          </div>
  ...
  likedBtn.forEach((el) => {
    el.addEventListener('click', (e) => {
      ...
      const data = { // формируем объект со всеми данными о товаре, чтобы не передавать каждое свойство по одному
        ...
        price: parent.querySelector('.product-price').innerHTML,
